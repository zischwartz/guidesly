...
def CardDetailView (request, gslug, slug):
	...
	interactive_elements=card.interactiveelement_set.all().select_subclasses()
...
def BuildCard (request, gslug):
	s = Card(guide=get_object_or_404(Guide, slug=gslug), title="Card Title")
	s.save()
...
def EditCard (request, gslug, id):
	if request.method == 'POST':
		...
		ur = CardResource()
		cardr = ur.get_detail(request, id=s.id)
		# cardr = ur.obj_get(id=s.id)
		ur_bundle = ur.build_bundle(obj=cardr, request=request)
		card_json= ur.serialize(None, ur.full_dehydrate(s), 'application/json')
